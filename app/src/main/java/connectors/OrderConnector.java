...
public class OrderConnector {
    ...
    private Order parseOrder(DataSnapshot snapshot) {
        ...
        order.userId = String.valueOf(snapshot.child("customer_id").getValue());// Điều chỉnh nếu userId không trùng customer_id
        order.status = snapshot.child("order_status").getValue(String.class);
        order.createdAt = snapshot.child("order_createdat").getValue(String.class);
        order.totalAmount = snapshot.child("total_amount").getValue(Double.class);
 ...
 //           item.setImageUrl(""); // fallback nếu không có ảnh
        ...
        if (paymentSnap.exists()) {
            ...
            paymentInfo.amount = paymentSnap.child("amount").getValue(Double.class);
